...
| `POST` | `/api/ai/summarize` | `{table, note?}` → 3줄 요약 + 최대 변화 + 예상질문 3개 |
...
48개 테스트입니다. `test_sync_server.py` 37개가 코드 발급·정규화, 저장소 왕복,
낙관적 동시성 충돌, 크기 제한, 유니코드 왕복, 재기동 후 데이터 보존, HTTP 계층
상태코드, 속도 제한을 덮습니다.

`test_ai.py` 11개는 실제 API 키 없이 돕니다 — SDK를 대역으로 갈아끼워 모델·구조화
