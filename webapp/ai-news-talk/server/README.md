...
| `POST` | `/api/ai/rival-brief` | 카드 4 — `{text, ours}` → 위협·과장·대응안(즉시/단기/중장기) |
| `POST` | `/api/ai/mail-tones` | 카드 6 — `{subject, facts, to, ask}` → 3종 메일 + 대외 리스크 |
| `POST` | `/api/ai/recheck` | 카드 7 — `{answer}` → 전제·정의·출처 지적 + 확신도 |
...
여덟 카드 전부 실행 가능해졌습니다. 각 카드가 **서버 없이 무엇을 할 수 있는지**가
설계의 중심입니다 — 흉내낼 수 없는 것은 흉내내지 않고 그렇다고 밝힙니다.

| 카드 | 서버 있을 때 | 서버 없을 때 |
|---|---|---|
| 4. 경쟁사 발표 | 위협·과장·대응안 | **로컬 분류** — 검증 가능한 주장 / 수식어 / 아직 안 된 것 |
| 6. 고객 메일 3종 | AI가 문장을 씀 | **문틀 조립** + 대외 리스크 검사 |
| 7. 수치 검산 | 전제·정의 교차 확인 | **로컬 검산이 정답** — 아래 참고 |
| 8. 보안 점검 | (쓰지 않음) | **로컬 전용** — 설계상 서버를 타지 않음 |

**카드 7이 유일하게 로컬이 AI보다 낫습니다.** 산수는 계산하면 끝나는 일이라
모델에게 다시 묻지 않습니다. 붙여넣은 답변에서 다섯 종류의 계산 주장을 찾아
브라우저가 직접 다시 계산합니다 — 사칙연산(`A + B = C`), 비율 적용(`A의 N%`),
증감률, 표의 합계 줄, 한 문장 안의 구성비 합. 정수는 정확히 맞아야 하고 비율은
0.5%p까지 `반올림 차이`로 구분합니다. 서버는 산수를 다시 따지지 않고 **전제·정의·
출처**만 봅니다(기간 정의가 도중에 바뀌지 않았는지, 비교 대상이 같은 기준인지).
둘이 다르면 로컬이 정답이라고 화면에서 밝힙니다.

오탐을 막는 데 신경을 썼습니다. 합계 검사는 **숫자가 정확히 하나인 줄**(표 형태)만
항목으로 인정합니다 — 산문 줄에서 마지막 숫자를 주워 오면 없는 오류를 만들어 내고,
검산 도구가 그러면 있는 것보다 나쁩니다. 구성비 합도 문서 전체가 아니라 한 문장
안의 나열만 봅니다.

**카드 8은 서버를 쓰지 않습니다.** 점검 대상이 민감정보 그 자체이므로, 검사하려고
서버에 보낸다면 카드가 가르치려는 것과 정반대입니다. 서버 주소가 설정돼 있어도
이 카드만은 전송하지 않으며, 테스트로 그 사실을 고정해 두었습니다. 11종을 찾습니다 —
주민번호·카드번호·API 키·비밀번호 기재·여권번호(위험), 휴대전화·사업자번호·계좌번호·
이메일·사내 IP·이름+직함(주의). 좁은 규칙이 먼저 자기 몫을 가져가도록 순서를 정해,
휴대폰 번호가 계좌번호로 중복 신고되지 않습니다. 찾아낸 값은 원문 전체를 보여주지
않고 `010***8`처럼 일부만 남기며, 가린 버전을 복사할 수 있습니다.
...
98개 테스트입니다. `test_sync_server.py` 37개가 코드 발급·정규화, 저장소 왕복,
낙관적 동시성 충돌, 크기 제한, 유니코드 왕복, 재기동 후 데이터 보존, HTTP 계층
상태코드, 속도 제한을 덮습니다.

`test_ai.py` 61개는 실제 API 키 없이 돕니다 — SDK를 대역으로 갈아끼워 모델·구조화
...
"지어내지 마라"·"문서에 없음"이 실제로 들어 있는지를 고정합니다. 카드 4·6·7은
관점이 프롬프트에 실리는지, 메일 프롬프트가 "사실에 없는 약속은 절대 넣지 마라"를
담는지, 검산 프롬프트가 "산수가 맞는지 다시 따지지 마라"로 산수를 클라이언트에
넘기는지를 고정합니다.

> 속도 제한(IP당 분당 60건)이 테스트 사이로 흘러넘쳐, 테스트가 늘면 뒤쪽이 `429`를
> 받습니다. `test_ai.py`는 매 테스트 앞뒤로 제한기를 비우는 autouse 픽스처를 둡니다.
