...
지시문 **조립과 점검은 브라우저에서** 합니다. 5칸(업무명·역할·입력·출력·금지)을
채우면 4단 구조 지시문이 만들어지고, 곧바로 4가지를 점검합니다 — 금지사항 개수,
출력 항목 개수, 출력 형식 명시 여부, 모호한 표현(`알아서`, `적절히`, `필요시` 등).
모호어는 모델이 알아서 메우는 지점이라, 나중에 품질 문제로 돌아옵니다.
...
62개 테스트입니다. `test_sync_server.py` 37개가 코드 발급·정규화, 저장소 왕복,
낙관적 동시성 충돌, 크기 제한, 유니코드 왕복, 재기동 후 데이터 보존, HTTP 계층
상태코드, 속도 제한을 덮습니다.

`test_ai.py` 25개는 실제 API 키 없이 돕니다 — SDK를 대역으로 갈아끼워 모델·구조화
