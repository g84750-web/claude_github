...
const KPI = (() => {
  ...
  const PILLARS = [
    ...
    { no: 6, name: 'Delivery Risk & Quality', ko: '납기 리스크 · 품질',
      desc: '성격이 다른 두 축을 분리해 본다. ' +
            '① 물량·납기 리스크 — 이월 물량(6.1), 지연 프로젝트(6.3), H/W 조달(6.4), 구축지연 M(6.5). ' +
            '② 품질 — 개통 후 안정성(6.2). ' +
            '이월 물량은 접수 집중에 따른 CAPA 리스크이지 품질 지표가 아니다.' },
  ...
  const LIST = [
    ...
    /* ═══════════ Pillar 6 ═══════════ */
    {
      id: '6.1', pillar: 6, name: '전년 이월 부담률 (물량 리스크)',
      def: '연초 잔여건 중 전년 이월 비중. 구축 CAPA 대비 접수 물량이 일시적으로 초과되어 발생하는 ' +
           '<b>물량·납기 리스크</b> 지표다. 품질 지표가 아니며, 이월 규모를 구축 품질 저하로 해석해서는 안 된다.',
      formula: '이월 부담률(%) = 전년 이월건 / 연초 전체 잔여건 × 100\n연초 전체 잔여건 = 전년이월 + 당해연도 1월 신규접수',
      source: 'GCMS: 구축접수일(Z열) 연도',
      cycle: '연 1회 (1월 첫째 주) · 재공 추이는 월간',
      target: '재공(WIP) 감소 추세 유지', unit: '%',
      asIs: '81.3% (2026년 1월)', state: 'auto',
      note: '⚠ 해석 주의 — 2025년 하반기 <b>업셀 전환 프로모션</b>으로 구축인원 대비 약 3배수 접수가 집중되었고, ' +
            '그 물량이 자연 이월된 것이다(이월 919건 중 99.5%가 2025년 접수). ' +
            '2026년 들어 완료가 접수를 상회하며 재공이 정상 소화 중이므로 <b>구축지연(6.5) 리스크로만 해석</b>하고, ' +
            '품질 저하나 처리 능력 부족으로 해석하지 않는다. 목표치(구 60% 이하)는 물량 특성상 관리 기준으로 부적합하여 제거했다.',
      calc: D => {
        const carry = D.meta.carry || 0;
        const yr = D.asOf.slice(0, 4);
        const janNew = D.rows.filter(p => p.recvYM === `${yr}-01`).length;
        const v = pct(carry, carry + janNew);
        // 재공(WIP) 소화 추이 — 이월이 정상 소화 중인지 확인
        const ms = D.monthly.filter(m => m.ym >= `${yr}-01`);
        const first = ms[0], last = ms[ms.length - 1];
        const burn = (first && last) ? first.wip - last.wip : null;
        const cy = ms.filter(m => m.completed > 0);
        return {
          v, disp: f1(v) + '%',
          sub: `이월 ${carry.toLocaleString()}건 / 1월 신규 ${janNew.toLocaleString()}건 — 물량 리스크`,
          detail: [
            ...Object.entries(D.meta.carryByYear || {}).map(([y, c]) =>
              ({ label: `${y}년 접수`, value: `${c.toLocaleString()}건` })),
            { label: `${yr}년 재공(WIP) 추이`, value: first && last
              ? `${f0(first.wip)} → ${f0(last.wip)}건 (${burn > 0 ? '−' : '+'}${f0(Math.abs(burn))})` : '—' },
            { label: `${yr}년 월평균 접수 / 완료`, value: cy.length
              ? `${f0(avg(cy.map(m => m.received)))}건 / ${f0(avg(cy.map(m => m.completed)))}건` : '—' },
            { label: '판정', value: burn > 0 ? '완료 > 접수 — 이월 소화 중' : '재공 증가 — 점검 필요' },
          ]
        };
      }
    },
  ...
  function judge(k, v) {
    const measured = v !== null && v !== undefined && isFinite(v);
    // 목표치를 두지 않는 지표(물량·구성비 등)는 값이 있으면 '추세 관리'로 표기한다
    if (measured && k.targetVal === undefined) return { cls: 'idle', txt: '추세 관리' };
    if (!measured) return { cls: 'idle', txt: '측정 대기' };
    const t = k.targetVal;
